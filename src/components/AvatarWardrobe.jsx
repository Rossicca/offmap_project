        ...
        <div className="avatar-wardrobe-layout">
          ...
          <div className="avatar-look-editor">
            ...
            <div className="wardrobe-options" aria-label={wardrobeTabs.find((item) => item.id === tab)?.name}>
              {(tab === "avatar" ? avatars : wardrobeCatalog[tab].filter((item) => item.avatars?.includes(previewAvatar.id))).map((item) => <button key={item.id} type="button" className={(tab === "avatar" ? selectedAvatarId : look[tab]) === item.id ? "is-active" : ""} onClick={() => choose(item.id)} aria-pressed={(tab === "avatar" ? selectedAvatarId : look[tab]) === item.id} style={{ "--swatch": item.color || "#eee4cb", "--swatch-accent": item.accent || item.color || "#8ca4a8" }}>{tab === "avatar" ? <i className="wardrobe-avatar-thumb" style={{ backgroundImage: `url(${item.motionSprite})` }} aria-hidden="true" /> : <i className="wardrobe-outfit-thumb" style={{ backgroundImage: `url(${item.sprite || previewAvatar.motionSprite})`, ...(item.maskWithBase ? { WebkitMaskImage: `url(${previewAvatar.motionSprite})`, WebkitMaskPosition: "0% 0%", WebkitMaskRepeat: "no-repeat", WebkitMaskSize: "200% 200%", maskImage: `url(${previewAvatar.motionSprite})`, maskPosition: "0% 0%", maskRepeat: "no-repeat", maskSize: "200% 200%" } : {}) }} aria-hidden="true" />}<b>{item.name}</b>{tab === "avatar" && <small>{item.gender}</small>}</button>)}
            </div>
          </div>
        </div>
        <footer>
