  ...
  return <>
    ...
    {open && <div className="music-player-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
      <section className="music-player" role="dialog" aria-modal="true" aria-labelledby="music-player-title">
        ...
        <div className="music-quick-settings"><b>当前画面</b><div><button type="button" onClick={() => openWorldTool("arrange")}>位置和大小</button><button type="button" onClick={() => openWorldTool("materials")}>加东西</button><button type="button" onClick={() => openWorldTool("house")}>装房子</button></div></div>
