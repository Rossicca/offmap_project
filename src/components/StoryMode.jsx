  ...
  return (
    <>
      ...
      {active && <section className="story-card" aria-label="故事任务">
        ...
        {ending ? <div className="story-ending"><strong>{ending === "night" ? "星光结局" : "晨光结局"}</strong><p>{ending === "night" ? "你和伙伴在星空下找到了会发光的友情种子！" : "太阳升起时，地图变成了一只金色纸飞机！"}</p><button type="button" onClick={onToggle}>回到自由世界</button></div> : <div className="story-step"><small>当前任务</small><h2>{current.title}</h2><p>{current.detail}</p>{current.choices ? <div className="story-choices"><button type="button" onClick={() => onAction("world", "sunset")}>进入星光夜晚</button><button type="button" onClick={() => onAction("world", "sunrise")}>迎接新的清晨</button></div> : <button className="story-action" type="button" onClick={() => onAction(current.target, current.action)}>{current.cta}<span>→</span></button>}</div>}
      </section>}
    </>
  );
}
